...
(function () {
    ...
    function check_position(f1, f2) {
        ...
        if ((diff_col == -1 && diff_x < 0 && abs(diff_x + w) < SNAP_DST || diff_col == 1 && diff_x >= 0 && abs(diff_x - w) < SNAP_DST) && (diff_y <= SNAP_DST && diff_y >= -SNAP_DST)) {
            var audio = new Audio('sound/find.mp3');
            //custom
            contestDetail.fixedPieces++;
            contestDetail.lastTime = new Date().getTime();
            audio.play();
            console.log("DANGGGGGGGGGGGGGGGGGGGGGGG");
...
(function () {
    ...
    var IN = "inside",
        ...
        default_opts = {
            ...
            maxWidth: 705,
            maxHeight: 470,
            defaultImage: "images/puzzle/hacik.jpg",
    ...
    jigsaw.Jigsaw = Class.extend({
        init: function (opts) {
            ...
            eventBus.on(jigsaw.Events.JIGSAW_COMPLETE, function () {
                var audio = new Audio('sound/bitdi.mp3');
                audio.play();
                console.log("completeeeeeee");
                ...
                if (contestRoom != null) {
                    finishInTime(contestDetail);
                } else {
                    //for levels
                    let id = document.getElementById("infoid").innerText;
                    var xhr = new XMLHttpRequest();
                    xhr.open('POST', 'http://35.231.39.26:3003/nextmission', true);
                    xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
                    xhr.onload = function () {
                        // do something to response
                        var xhrr = new XMLHttpRequest();
                        xhrr.open('GET', 'reclamprofile', true);
                        xhrr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
                        xhrr.send('user=person');
                        console.log(this.responseText);
                    };
                    xhr.send('user=person');

                    setTimeout(function () {
                        window.location.href = "http://35.231.39.26:3003/?_id=" + id
                    }, 5000);
                    self.ui.stop_clock();
                }
        ...
        shuffle: function () {
            ...
            for (i = 0; i < l; i++) {
                ...
                F.tx = Util.randint(width);
                F.ty = Util.randint(height);

                // console.log(F.tx, F.ty);
    ...
    function jigsaw_events(ce, eventBus, rotate) {
        ce.drag("*", {
            ...
            afterMove: function (c, e) {
                ...
                var audio = new Audio('sound/endir.mp3');
                audio.play();
